...
export const updateChannelList = (
    ...
    while (i < m3uArray.length) {
        const line = m3uArray[i]
        if (line.startsWith("#EXTINF")) {
            const reg = channelRegExp.exec(line)
            if (reg) {
                // Find corresponding URL (skip EXTVLCOPT)
                let j = i + 1
                while (j < m3uArray.length && m3uArray[j].startsWith("#EXTVLCOPT")) {
                    j++
                }
                if (j < m3uArray.length) {
                    channels.push([
                        reg[2].replace(/\|/g, "").trim(),
                        get_from_info(m3uArray[j]),
                        m3uArray[j],
                    ])
                    i = j + 1
                    continue
